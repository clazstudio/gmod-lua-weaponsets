...
local function playersListMenu(plyList, callback)
    plyList:AddOption("All online players", function() callback() end):SetIcon("icon16/group.png")
    plyList:AddOption("Myself", function() callback(LocalPlayer()) end):SetIcon("icon16/user.png")
    plyList:AddSpacer()

    for _, v in pairs(player.GetAll()) do
        plyList:AddOption(v:Nick(), function() callback(v) end)
    end
    return plyList
end
...
local function buildModifyPanel(pan)
    ...
    function ls:OnRowRightClick(ind, line)
        ...
        acts:AddOption("Edit...", function() RunConsoleCommand("weaponsets", name) end):SetIcon("icon16/page_edit.png")
    ...
    newSetPanel:SetDrawBackground(false)
...
list.Set("DesktopWindows", "CLazWeaponSets", {
	...
	init = function(icon, window)
        ...
        for _, name in pairs(WEAPONSETS.WeaponSetsList) do
            ...
            subMenu:AddSpacer()
            subMenu:AddOption("Give to all", function() RunConsoleCommand("weaponsets_give", name) end):SetIcon("icon16/group_go.png")
